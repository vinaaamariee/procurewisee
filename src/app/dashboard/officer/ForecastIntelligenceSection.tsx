    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginTop: '0.25rem' }}>
              {forecastData.expectedToDecrease.length === 0 ? (
                ...
                forecastData.expectedToDecrease.slice(0, 3).map(p => (
                  <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.75rem' }}>
                    <span style={{ fontWeight: 600, color: v.textPrimary }}>{p.name}</span>
                    <span style={{ fontWeight: 800, color: v.green, background: 'rgba(128, 0, 0, 0.05)', padding: '0.1rem 0.4rem', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '2px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'space-between' }}>
          ...
          <div style={{ background: 'rgba(128, 0, 0, 0.05)', border: `1px solid rgba(128, 0, 0, 0.12)`, borderRadius: '0.75rem', padding: '0.75rem 1rem', display: 'flex', alignItems: 'start', gap: '0.5rem' }}>

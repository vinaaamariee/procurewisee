...
- **Procurement Officer Dashboard Redesign & Polish (`/dashboard/officer`)**: Overhauled to look like an official Philippine government/university portal. Removed SaaS decorative widgets, global search, Recent Activity timeline, and detailed health status footers. Replaced the large hero banner with a clean title and status bar detailing user name, role, and current date (without conversational greetings, keeping high text contrast). Removed breadcrumbs. KPI cards are styled as light-themed containers using subtle borders (`border-base-300`), small shadows (`shadow-sm`), and rounded corners (`rounded-xl`), with visual description clutter removed. The "Create RFQ" button is converted to a subtle outlined button to reduce visual dominance. The "Today's Tasks" section features a cleaner, light appearance with subtle priority indicator dots instead of loud badges. The Settings navigation link points to the newly created `/dashboard/officer/settings` route.

...
- **Procurement Officer Dashboard Redesign (`/dashboard/officer`)**: Overhauled to look like an official Philippine government/university portal. Removed SaaS decorative widgets, global search, Recent Activity timeline, and detailed health status footers. Replaced the large hero banner with a clean title and welcome message detailing user name, role, and current date. Features a single primary "Create RFQ" button. Restored the **Forecast Intelligence** section showing the ARIMA forecasting tool immediately below the Today's Tasks area. Grouped the sidebar navigation cleanly (Dashboard, Purchase Requests, Annual Procurement Plan, Requests for Quotation, Purchase Orders, Suppliers, Reports, Settings); to prevent any 404 errors, placeholder/unimplemented routes (Annual Procurement Plan, Settings) are pointed to `#`, visually styled as disabled (opacity 40%, cursor-not-allowed), and have clicks prevented programmatically.

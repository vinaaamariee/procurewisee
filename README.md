...
- **BSC Institutional Landing Page (`/`)**: Redesigned official public entry point matching the login page theme (`bsc` maroon `#7B1E1E`, gold `#A6761D`, white, and light gray) and Batanes State College branding. Features a sticky daisyUI navbar, institutional hero section, 7-step procurement lifecycle (`steps`), core module cards (`card`), user roles breakdown (`End User`, `Procurement Officer`, `BAC Secretariat`, `Administrator`), and official college footer compliant with Republic Act No. 9184.

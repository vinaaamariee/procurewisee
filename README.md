...
- **Procurement Officer Dashboard Redesign (`/dashboard/officer`)**: Overhauled to look like an official Philippine government/university portal. Removed SaaS decorative widgets, global search, forecast intelligence, Recent Activity timeline, and detailed health status footers. Replaced the large hero banner with a clean title and welcome message detailing user name, role, and current date. Features a single primary "Create RFQ" button. The sidebar navigation links (Dashboard, Purchase Requests, Annual Procurement Plan, RFQs, Purchase Orders, Reports & Insights, Forecast Intelligence, Recent Solicitations, Supplier Profiles) are restored exactly to their original `href` paths to avoid 404 errors. Consolidates the KPI cards into four key operational metrics (Pending Purchase Requests, Open RFQs, Pending Purchase Orders, Active Suppliers) and features a prioritized list of Today's Tasks (originator and priority color-coding) and simplified Recent Solicitations table data.

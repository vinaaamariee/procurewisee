...
- **`officer/history/VerificationHistoryClient.tsx`**: Table updated to match spec:
  ...
  - Added **"Action Taken"** column — color-coded pill badge (Verified: green, Returned for Revision: amber, Rejected: red)
  - Added **"Remarks"** column showing officer remarks for returned/rejected PRs
  - CSV export updated to include new columns
- **`officer/history/page.tsx`**: Added `remarks` to Prisma select; updated metadata title.
- **`officer/reports/page.tsx`**: Updated metadata title.
...
### Root Cause (Phase 1 — Vertical Overflow)
...
Establishes the formal validation and approval gate between the End User and the Procurement Office following the actual procurement process of Batanes State College. Only Purchase Requests that have successfully passed Procurement Office validation become eligible for RFQ generation, ensuring compliance with institutional rules.

### Workflow Sequence
```
End User (Create & Submit PR)
      ↓
Status: Pending Procurement Review (Amber Badge)
      ↓
Procurement Officer Review & 5-Point Validation Checklist
   ├── Approve (Passing 5/5 Checks) → Status: Approved | Eligible for RFQ
   └── Return (Mandatory Comment) → Status: Returned
         ↓
End User Revises & Resubmits → Status: Pending Procurement Review
```
...
- **BSC Institutional Login Portal (`/login`)**: Redesigned as a secure **authentication gateway** complementing the landing page. Left panel shows compact institutional branding (BSC logo, "Official Portal" badge, "Powered by ProcureWise") and 4 security/access feature cards (`Secure Authentication`, `Role-Based Access Control`, `Institutional Procurement System`, `RA 9184 Compliant Workflow`) replacing the full workflow steps. Includes "New to ProcureWise?" → Create Account CTA. Right panel (`LoginForm.tsx`) adds `← Back to Home` link above card, and Role Access Summary updated to schema-confirmed roles only (`End User`, `Procurement Officer`, `Administrator`) — BAC Secretariat removed. `LoginFooter.tsx` now includes a "Back to Home" link.
- **BSC Institutional Landing Page (`/`)**: Redesigned as the official **internal** public entry point for the ProcureWise PMIS. No public procurement catalog exposure. Navigation limited to informational anchors (`Workflow`, `Features`, `About`, `Help`). Auth-gate notices on Hero and all module cards. Primary CTAs are `Sign In` and `Create Account`. User Roles section sourced from `UserRole` Prisma enum (`EndUser`, `ProcurementOfficer`, `Administrator`). New `AboutSection.tsx` and `HelpSection.tsx` (with daisyUI collapse FAQ) added. `CatalogPreviewSection.tsx` removed.
- **403 Forbidden (Access Denied) Redesign (`/unauthorized`)**: Transformed the static placeholder page into a fully integrated, production-ready authorization gateway. It now enforces auth checks (unauthenticated users redirect to `/login`), displays dynamic and accurate roles (compares `Current Role` vs `Required Role` in a side-by-side card container), has a note directing users to contact support, and features functional navigation (`Back to Dashboard` links to the home of their actual authenticated role, and `Sign in with a different account` destroys the Supabase session/cookies). Fully integrated with the proxy route guard redirects.
- **Procurement Officer Dashboard Redesign & Polish (`/dashboard/officer`)**: Overhauled to look like an official Philippine government/university portal. Removed SaaS decorative widgets, global search, Recent Activity timeline, and detailed health status footers. Replaced the large hero banner with a clean title and status bar detailing user name, role, and current date (without conversational greetings, keeping high text contrast). Removed breadcrumbs. KPI cards are styled as light-themed containers using subtle borders (`border-base-300`), small shadows (`shadow-sm`), and rounded corners (`rounded-xl`), with visual description clutter removed. The "Create RFQ" button is converted to a subtle outlined button to reduce visual dominance. The "Today's Tasks" section features a cleaner, light appearance with subtle priority indicator dots instead of loud badges. The Settings navigation link points to the newly created `/dashboard/officer/settings` route. The dashboard is simplified to display only the header, 4 KPI cards, Today's Tasks, and the Recent RFQs table.
- **Dedicated Forecasting Page (`/dashboard/officer/forecast`)**: Removed the Forecasting section from the main dashboard and created a dedicated route under the `REPORTS` sidebar section (Reports -> Forecasting). The new page displays tab-level placeholders for the ARIMA predictive chart and demand model visualizations.
- **System Settings Placeholder Page (`/dashboard/officer/settings`)**: Created a settings page returning valid JSX. It displays tab layout options (General Config, Security & Access, Notifications) with a clean card explaining that the Settings module is currently under development, ensuring the app remains error-free and has a professional institutional fallback.
- **Purchase Request Detail Page Bug Fix (`/dashboard/officer/pr/[id]`)**: Resolved a page rendering crash ("Failed to load Requisition Details" error screen) caused by passing raw Prisma `Decimal` objects and serialized Date parameters from the Server Component to `PrDetailsClient`. Added a serializing wrapper to maps Decimals to numbers and Dates to ISO strings before component mounting, and included the `unit` (UnitOfMeasure) relation query to map it safely to its string abbreviations (preventing React child rendering crashes).
- **Purchase Order Workspace & DB Synchronization Bug Fix (`/dashboard/officer/po`)**: Fixed a global error boundary crash on the PO Drafting Workspace by mapping Decimal scores/amounts and Date instances to plain JSON values prior to rendering the `PoDraftingClient` Client Component. Corrected inconsistent database records in `purchase_request_status_history` where string statuses were stored unmapped (e.g. `'UnderReview'`), synced columns (like `requests_for_quote.prId` and `purchase_orders.updatedAt`), and pushed the fully aligned schema using the direct PostgreSQL session port `5432`.

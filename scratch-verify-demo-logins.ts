...
const accounts = [
  ...
  { role: 'Supplier', email: 'supplier@procurewise.local', password: 'password123' }
];
...
async function verifyDemoAccounts() {
  ...
  for (const acc of accounts) {
    ...
    if (error) {
      ...
      console.log(`✅ [${acc.role}] ${acc.email} SUCCESS (Profile Role: ${profile?.role}, FullName: ${profile?.fullName})`);
    }
  }
